...
class AttBiRNN(object):
    ...
    def __init__(self, sequence_length, num_classes, channel_num, rnn_hidden_size, attention_size):
        ...
        with tf.name_scope("output"):
            ...
            self.labels = tf.argmax(self.scores, 1, name="labels")
        ...
        with tf.name_scope("accuracy"):
            correct_predictions = tf.equal(self.labels, tf.argmax(self.input_y, 1))
...
class MLP(object):
    ...
    def __init__(self, sequence_length, num_classes, channel_num, hidden_size):
        ...
        # FC layer
        with tf.name_scope("output"):
            FC_W1 = tf.get_variable("FC_W1", shape=[sequence_length * channel_num, hidden_size],
                                    initializer=tf.contrib.layers.xavier_initializer())
            FC_b1 = tf.Variable(tf.constant(0.1, shape=[hidden_size]), name="FC_b1")
            self.fc_out1 = tf.nn.xw_plus_b(tf.reshape(self.input_x, [-1, sequence_length * channel_num]),
                                           FC_W1, FC_b1, name="FC_out1")
            FC_W2 = tf.get_variable("FC_W2", shape=[hidden_size, num_classes],
                                    initializer=tf.contrib.layers.xavier_initializer())
            FC_b2 = tf.Variable(tf.constant(0.1, shape=[num_classes]), name="FC_b2")
            self.fc_out2 = tf.nn.xw_plus_b(self.fc_out1, FC_W2, FC_b2, name="FC_out2")

            self.scores = tf.nn.softmax(self.fc_out2, name='scores')
            self.labels = tf.argmax(self.scores, 1, name="labels")

        with tf.name_scope("loss"):
            losses = tf.nn.softmax_cross_entropy_with_logits(logits=self.fc_out2, labels=self.input_y)
            self.loss = tf.reduce_mean(losses, name='loss')
